...
impl<T> UnorderedArrayLikeChange<T> {
    fn new(item: T, count: usize, insert_or_remove: InsertOrRemove) -> Self {
        ...
        match (insert_or_remove, count) {
            ...
            (InsertOrRemove::Insert, val) if val <= u8::MAX as usize => UnorderedArrayLikeChange::InsertFew(UnorderedArrayLikeChangeSpec{ item, count: val as u8 }),
            (InsertOrRemove::Insert, val) if val > u8::MAX as usize => UnorderedArrayLikeChange::InsertMany(UnorderedArrayLikeChangeSpec{ item, count: val }),
            (InsertOrRemove::Remove, 1) => UnorderedArrayLikeChange::RemoveSingle(item),
            (InsertOrRemove::Remove, val) if val <= u8::MAX as usize => UnorderedArrayLikeChange::RemoveFew(UnorderedArrayLikeChangeSpec{ item, count: val as u8 }),
            (InsertOrRemove::Remove, val) if val > u8::MAX as usize => UnorderedArrayLikeChange::RemoveMany(UnorderedArrayLikeChangeSpec{ item, count: val }),
            (_, _) => unreachable!()
        }
    }
}
    ...
    for (&k, current_count) in current.iter() {
        match previous.remove(&k) {
            Some(prev_count) => match (*current_count as i128) - (prev_count as i128) {
                add if add > 1 => ret.push(UnorderedArrayLikeChange::new(k.clone(), add as usize, InsertOrRemove::Insert)),
                add if add == 1 => ret.push(UnorderedArrayLikeChange::new(k.clone(), add as usize, InsertOrRemove::Insert)),
                sub if sub < 0 => ret.push(UnorderedArrayLikeChange::new(k.clone(), -sub as usize, InsertOrRemove::Remove)),
                sub if sub == -1 => ret.push(UnorderedArrayLikeChange::new(k.clone(), -sub as usize, InsertOrRemove::Remove)),
                _ => (),
            },
            None => ret.push(UnorderedArrayLikeChange::new(k.clone(), *current_count, InsertOrRemove::Insert)),
        }
    }

    for (k, v) in previous.into_iter() {
        ret.push(UnorderedArrayLikeChange::new(k.clone(), v, InsertOrRemove::Remove))
    }

    match ret.is_empty() {
        true => None,
        false => Some(UnorderedArrayLikeDiff(UnorderedArrayLikeDiffInternal::Modify(ret))),
    }
}
...
>(
    ...
    let (insertions, removals): (Vec<UnorderedArrayLikeChange<T>>, Vec<UnorderedArrayLikeChange<T>>) =
        diffs.into_iter().partition(|x| match &x {
            UnorderedArrayLikeChange::InsertMany(_) | UnorderedArrayLikeChange::InsertFew(_) | UnorderedArrayLikeChange::InsertSingle(_) => true,
            UnorderedArrayLikeChange::RemoveMany(_) | UnorderedArrayLikeChange::RemoveFew(_) | UnorderedArrayLikeChange::RemoveSingle(_) => false,
        });
...
#[cfg(feature = "nanoserde")]
mod nanoserde_impls {
    use super::{DeBin, UnorderedArrayLikeChangeSpec, SerBin, UnorderedArrayLikeDiff, UnorderedArrayLikeDiffInternal, UnorderedArrayLikeChange};
    ...
    impl<T: DeBin + PartialEq + Clone + SerBin> DeBin for UnorderedArrayLikeDiff<T> {
        fn de_bin(
            ...
            let id: u8 = DeBin::de_bin(offset, bytes)?;
            core::result::Result::Ok(match id {
                0_u8 => UnorderedArrayLikeDiff(UnorderedArrayLikeDiffInternal::Replace(DeBin::de_bin(offset, bytes)?)),
                1_u8 => UnorderedArrayLikeDiff(UnorderedArrayLikeDiffInternal::Modify(DeBin::de_bin(offset, bytes)?)),
                _ => {
                    return core::result::Result::Err(nanoserde::DeBinErr {
                        o: *offset,
                        l: 0,
...
mod test {
    ...
    use super::{UnorderedArrayLikeDiffInternal, UnorderedArrayLikeDiff};
    ...
    fn test_collection_strategies() {
        ...
        if let TestCollectionFields::test1(
            UnorderedArrayLikeDiff(UnorderedArrayLikeDiffInternal::Replace(val)),
        ) = &diffs[0]
        {
            assert_eq!(val.len(), 0);
        } else {
        ...
        if let TestCollectionFields::test3(
            UnorderedArrayLikeDiff(UnorderedArrayLikeDiffInternal::Modify(val)),
        ) = &diffs[2]
        {
            assert_eq!(val.len(), 1);
        } else {
