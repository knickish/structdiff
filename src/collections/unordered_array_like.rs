...
fn collect_into_map<'a, T: Hash + PartialEq + Eq + 'a, B: Iterator<Item = T> + ExactSizeIterator>(
    list: B,
) -> HashMap<T, usize> {
...
) -> impl Iterator<Item = T> where <B as IntoIterator>::IntoIter: ExactSizeIterator {
